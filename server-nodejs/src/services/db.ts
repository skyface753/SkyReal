...
async function query(sql: string, params: any[]) {
	try {
		if (process.env.SQLDEBUG == 'true') {
			...
			console.log(
				'DB CONFIG: ' + JSON.stringify(config.SQLDB)
			);
		}
...
async function initDb() {
	...
	const reals =
		'CREATE TABLE IF NOT EXISTS `reals` (`id` INT NOT NULL AUTO_INCREMENT , `userFk` INT NOT NULL , `fronPath` VARCHAR(250) NOT NULL, `backPath` VARCHAR(250) NOT NULL , `createdAt` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`), CONSTRAINT `reals_user` FOREIGN KEY (`userFk`) REFERENCES `user`(`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4';
	const friendships =
	...
	if (process.env.SQLDEBUG == 'true') {
		...
		console.log(
			'User 2FA Result: ' + JSON.stringify(user2faResult)
		);
		console.log('Reals Result: ' + JSON.stringify(realsResult));
		console.log(
			'Friendships Result: ' +
				JSON.stringify(friendshipsResult)
		);
	}
	if (
		roleResult &&
